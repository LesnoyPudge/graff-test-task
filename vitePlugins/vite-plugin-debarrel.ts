// https://gist.github.com/developit/d2047ccb47d15b01486484fb79a2f51a
...
import { type ParseResult, type ExportSpecifier } from 'rs-module-lexer';
import type { Plugin, TransformResult } from 'vite';
import type { PluginContext } from 'rollup';
import rsModuleLexer from 'rs-module-lexer';


const { parse } = rsModuleLexer;


type DebarrelPluginOptions = {
...
function getImportNames(specifiers: string): ImportName[] {
    ...
    if (!specifiers.includes('{')) {
        const defaultMatch = defaultImportRegex.exec(specifiers);
        if (defaultMatch) {
    ...
    const defaultMatch = leadingTrailingDefaultRegex.exec(specifiers);
    if (defaultMatch) {
        names.push({
            imported: 'default',
...
function findMatchingImport(
    ...
    if (imp && !localExportName) {
        ...
        if (!slice.includes('*')) {
            const ln = (/([\w$]+)\s*as\s*$/.exec(slice))?.[1];
...
async function resolveThroughBarrel(
    ...
    for (const exp of exports) {
        ...
        if (!imp?.n) return { exportName, id, resolved: true };
...
export function debarrelPlugin(options: DebarrelPluginOptions = {}): Plugin {
    ...
    return {
        ...
        async transform(code, id) {
            ...
            if (options.include && !options.include.some((pattern) => id.match(pattern))) return;
